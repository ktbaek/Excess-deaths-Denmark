---
title: "Quarterly age and sex stratified death rates in Denmark from 2020 to 2022 (WIP)"
output:
...
```{r import-10y-data}
# Import data -------------------------------------------------------------
pop_quarterly_10 <- c19dk::get_pop_by_breaks(
  data = "data/tidy_DST_pop.csv",
  age_breaks = c19dk::get_age_breaks(maxage = 90, agesplit = 10)
)
...
death_rates_10 <- make_death_rates(deaths_10, pop_quarterly_10, Age, Sex)
...
```{r deaths-population, echo=FALSE, fig.asp=1, layout="l-page", fig.cap = "**Population size and number of deaths in each age group for the each quarter from 2010 Q1 to 2022 Q3.** (A) Deaths shown with fixed y-axis, (B) deaths shown with variable y-axes, and (C) population size."}
p1 <- death_rates_10 %>%
  filter(Year >= 2010) %>% 
  ggplot() +
  ...
  theme(
    ...
    plot.margin = margin(0.7, 0.7, 0.3, 0.7, "cm"),
...
preds %>%
  ...
  geom_line(data = function(x) subset(x, prediction == "2010-19"), aes(YQ, Death_rate * 1000, color = Sex), size = .3) +
  geom_point(data = function(x) subset(x, prediction == "2010-19"), aes(YQ, Death_rate * 1000, color = Sex), stroke = 0, size = 1) +
...
preds %>%
  ...
  ggplot() +
  geom_ribbon(aes(x = Quarter, ymin = conf_lo_Q * 100, ymax = conf_hi_Q * 100), fill = "gray70", alpha = 0.4) +
...
preds %>%
  ...
  ggplot() +
  geom_ribbon(aes(YQ, ymin = conf_lo * 1000, ymax = conf_hi * 1000), fill = "gray70", alpha = 0.4) +
  geom_line(aes(YQ, fit * 1000), size = .3) +
  geom_vline(xintercept = 2019.88, size = 0.4, color = "gray70") +
  geom_line(aes(YQ, Death_rate * 1000, color = Sex), size = .3) +
  gglayer_1 +
  theme(panel.spacing.y = unit(0.7, "cm"))
```


```{r sab-10y-sex-base2, echo=FALSE, fig.asp=1.3, layout="l-page", fig.cap = "**Quarterly death rates and season adjusted baselines using  2015-19 as reference period.** Colored lines indicate the quarterly death rates, black lines indicate the season adjusted baselines, and the gray band indicate the 95% prediction interval. Sex is indicated on top, age on the right."}
...
p2 <- preds %>%
  ...
  theme(
    ...
    plot.margin = margin(0.7, 0.7, 0.3, 0.2, "cm"))
...
p1 <- preds %>%
  ...
  geom_line(aes(YQ, (fit  - fit) * 1000), size = .3) +
  geom_line(aes(YQ, (Death_rate - fit) * 1000, color = Sex), size = .5) +
  gglayer_1 +
  scale_y_continuous(limits = c(NA, NA)) +
...
p2 <- preds %>%
  ...
  facet_wrap(~ Age, ncol = 5, scales = "free_y") +
  labs(y = expression(bold(Delta*"death rate (per 1000)"))) +
  theme(plot.margin = margin(0.3, 0.7, 0.3, 0.7, "cm"))
...
```{r excess-both-bases-quarter-age, echo=FALSE, fig.asp=0.6, layout="l-body-outset", fig.cap = "**Excess deaths by age and quarter.** Excess deaths are defined as deaths exceeding the season adjusted baseline +/- the 95% prediction interval in a given quarter. Age groups 0-19 are ommitted."}
...
preds %>%
  ...
  knitr::kable(caption = "Excess deaths for each age group, sex and baseline for the period 2020 Q1 - 2022 Q3. Rounded to whole numbers.")
```

```{r excess-both-bases-quarter-sex, echo=FALSE, fig.asp=0.55, layout="l-body-outset", fig.cap = "**Excess deaths by sex and quarter.** Excess deaths are defined as deaths exceeding the season adjusted baseline +/- the 95% prediction interval in a given quarter."}
...
```{r excess-both-bases-age-sex, echo=FALSE, fig.asp=0.55, layout="l-body-outset", fig.cap = "**Excess deaths by sex and age.** Excess deaths are defined as deaths exceeding the season adjusted baseline +/- the 95% prediction interval in a given quarter. Total excess deaths over the period 2020 Q1 - 2022 Q3 are summed. "}
preds %>%
  unnest(c(predict, excess)) %>%
  filter(Year >= 2020) %>%
  ggplot() +
...
```{r excessrate-both-bases-year, echo=FALSE, fig.asp=0.5, fig.cap = "**Excess deaths per 100,000 of the total Danish population per year.** Excess deaths are defined as deaths exceeding the season adjusted baseline +/- the 95% prediction interval for a given stratification in a given quarter. *) 2022 is calculated based on Q1, Q2, and Q3 only. The population for each year is taken as the mean of the population at the start of each quarter."}
preds %>%
  unnest(c(predict, excess)) %>%
...
```{r excessrate-more-bases-10y-sex, echo=FALSE, layout = "l-body_outset", fig.asp=1.1, fig.cap = "**Excess deaths for the period 2020 Q1 - 2022 Q3 depending on baseline reference period.** Excess deaths are defined as deaths exceeding the season adjusted baseline +/- the 95% prediction interval in a given quarter. Baseline period is indicated on the x-axis. (A) Excess deaths per 100,000 of the total Danish population, and (B) excess deaths stratified by age."}
p1 <- preds_more %>%
  unnest(c(predict, excess)) %>%
  left_join(pop_total, by = "YQ") %>%
  filter(Year >= 2020) %>%
...
```{r calc-deaths-pop-5y}
# Import data -------------------------------------------------------------
pop_quarterly_5 <- c19dk::get_pop_by_breaks(
  data = "data/tidy_DST_pop.csv",
  age_breaks = c19dk::get_age_breaks(maxage = 95, agesplit = 5)
)
...
death_rates_5 <- make_death_rates(deaths_5, pop_quarterly_5, Age, Sex) 
...
death_rates_ns <- make_death_rates(deaths_10, pop_quarterly_10)
...
death_rates_5a <- make_death_rates(deaths_5, pop_quarterly_5, Age)
death_rates_10a <- make_death_rates(deaths_10, pop_quarterly_10, Age)

# to display the age groups in the correct order in plots
death_rates_5a$Age <- factor(death_rates_5a$Age, levels = c(
  "0-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39",
  "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79",
...
```{r excessrate-all-preds, echo=FALSE,  layout = "l-body_outset", fig.asp=0.5, fig.cap = "**Excess deaths per 100,000 of the total Danish population for the period 2020 Q1 - 2022 Q3 for stratified and non-stratified data depending on baseline reference period.** Excess deaths are defined as deaths exceeding the season adjusted baseline +/- the 95% prediction in a given quarter. Baseline reference period is indicated on the x-axis."}
all_preds %>% 
  ggplot() +
...
```{r excessrate-mean-all-preds,  fig.asp=0.5,  echo=FALSE, layout="l-body_outset"}
all_preds %>%
  ggplot() +
